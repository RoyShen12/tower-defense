  ...
  function Game(GX = 36, GY = 24) {
    ...
    _defineProperty(this, "leftClickHandler", mousePos => {
      ...
      if (this.selectedTowerTypeToBuild) {
        const [mouseGposx, mouseGposy] = this.coordinateToGridIndex(mousePos); // 不在左侧区域
        ...
        this.grids.forEach((rowv, rowi) => {
          rowv.forEach((colv, coli) => {
            if (!(mouseGposx === rowi && mouseGposy === coli) && colv === 1 && !(rowi === this.DestinationGrid.x - 1 && coli === this.DestinationGrid.y - 1)) {
              const c = new CircleBase(new Position((coli + .5) * this.gridSize, (rowi + .5) * this.gridSize), 5, 1, 'rgba(12,12,12,1)'); // c.grid = `${coli} ${rowi}`
      ...
      if (mousePos.x > this.midSplitLineX) {
        const selectedT = this.towerForSelect.find(tfs => tfs.position.equal(mousePos, tfs.radius)); // 选择建筑，进入修建状态
        ...
        /** @type {TowerBase}} */
        const selectedT = this.towerCtl.towers.find(t => t.position.equal(mousePos, t.radius)); // 升级
    ...
    _defineProperty(this, "mouseMoveHandler", _.throttle(e => {
      ...
      if (e.offsetX > this.midSplitLineX) {
        const selectedT = this.towerForSelect.find(tfs => tfs.position.equal(mousePos, tfs.radius)); // 鼠标进入建筑
    ...
    Game.callTowerList = () => this.towerCtl.towers; // 传奇宝石 升级点数
  ...
  _createClass(Game, [{
    ...
    value: function whichGrid(pos) {
      if (pos.x > this.midSplitLineX) return [-1, -1, NaN, NaN];else {
        const [ix, iy] = this.coordinateToGridIndex(pos); // console.log(pos.toString(), '->', `<${ix}, ${iy}>`)
    ...
    value: function getPathToEnd(startPos) {
      ...
      if (this.posPathMapping.has(key)) {
        ...
        const G = this.makeGraph(); //console.log(G.grid)
        // wg.x和y 各加1（寻路时的围墙）
        ...
        const path = Astar.astar.search(G, G.grid[wg[0] + 1][wg[1] + 1], G.grid[this.DestinationGrid.x][this.DestinationGrid.y]).map((p, idx) => {
          ...
          const x = (p.y - .5) * this.gridSize; // idx === 0 ? this.contextCtl._get_path_dbg.moveTo(x, y, this.gridSize, this.gridSize) :
    ...
    value: function placeTower(pos, ctorName, img, bimg, price) {
      const wg = this.whichGrid(pos); // console.log('wg', wg)
    ...
    value: function init() {
      ...
      this.DestinationPosition = new Position((this.DestinationGrid.x - 0.5) * this.gridSize, (this.DestinationGrid.y - 0.5) * this.gridSize);
      ...
      this.contextCtl.createCanvasInstance('off_screen_render', null, null, null, true); // [60 FPS] 常更新主图层

      this.contextCtl.createCanvasInstance('main', {
        zIndex: '2'
      }, null, null, false, null, 'off_screen_render'); // [stasis] 用来绘制塔的图层，不经常更新
      ...
      this.evtCtl.bindEvent([{
        ename: 'onmousedown',
        cb: e => {
          const mousePos = new Position(e.offsetX, e.offsetY); // console.log('mouse click axis: ' + mousePos)
    ...
    value: function renderOnce() {
      ...
      const tsMarginBottom = this.gridSize / 2 + 6; // console.log(`tsMargin=${tsMargin}, oneTsWidth=${oneTsWidth}, tsItemRadius=${tsItemRadius}, chunkSize=${chunkSize}, tsMarginBottom=${tsMarginBottom}`)
    ...
    value: function emitMoney(changing, happenedPosition) {
      this.money += changing; // if (happenedPosition) {
    ...
    value: function renderInformation() {
      ...
      const ay2 = ay1 - 30;
      const ay3 = ay2 - 30; // const ay4 = ay3 - 30
      ...
      this.contextCtl.refreshText(`总击杀: ${Tools.chineseFormatter(this.towerCtl.totalKill, 2, ' ')}`, null, new Position(ax, ay3), new Position(ax - 4, ay3 - 20), 190, 26, 'rgba(24,24,24,1)', true, '14px TimesNewRoman'); // this.contextCtl.refreshText(`CH: ${Tools.chineseFormatter(this.monsterCtl.totalCurrentHealth, 2, ' ')}`, null, new Position(ax, ay4), new Position(ax - 4, ay4 - 20), 190, 26, 'rgba(24,24,24,1)', true, '14px TimesNewRoman')
    }
  }, {
    key: "moneyOnDispaly",
    get: function () {
